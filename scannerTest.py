...
def detect_markers(frame):
    """Detect QR code and additional ArUco markers for better alignment."""
    ...
    detector = cv2.aruco.ArucoDetector(aruco_dict, aruco_params)  # Updated ArUco detection
...
def align_image_with_markers(image, mask, qr_corners, marker_corners, sheet_size, outline_position, outline_size):
    ...
    if marker_corners is not None and len(marker_corners) >= 4:
        ...
        all_corners = np.vstack((qr_corners, np.array(marker_corners[:4]).reshape(-1, 2)))
        expected_all_corners = np.vstack((expected_qr_corners, expected_marker_corners))
        
        M, _ = cv2.findHomography(all_corners, expected_all_corners, cv2.RANSAC, 5.0)
    else:
        M = cv2.getPerspectiveTransform(qr_corners, expected_qr_corners)
    
    # Apply homography transformation
    aligned_img = cv2.warpPerspective(image, M, sheet_size)
    aligned_mask = cv2.warpPerspective(mask, M, sheet_size)
    
    # Extract the correct region from the transformed image
    ...
    cropped_mask = aligned_mask[outline_y:outline_y + outline_size[1], outline_x:outline_x + outline_size[0]]
    
    # Resize to match the final output size (600x600)
    final_output = cv2.resize(cropped_img, (600, 600))
    final_mask = cv2.resize(cropped_mask, (600, 600))
...
def detect_qr_and_process(frame):
    ...
    if qr_data and qr_corners is not None:
        ...
        aligned_frame, aligned_mask = align_image_with_markers(frame, mask, qr_corners, marker_corners, sheet_size, outline_position, outline_size)
        masked_frame = apply_bw_mask(aligned_frame, aligned_mask)
